package com.figaf.integration.common.socket;
...
public class ConnectivitySocks5ProxySocket extends Socket {
    ...
    public ConnectivitySocks5ProxySocket(String jwtToken, String sccLocationId, String proxyHost, int proxyPort) {
        this.jwtToken = jwtToken;
        this.sccLocationId = StringUtils.isNotBlank(sccLocationId) ? Base64.getEncoder().encodeToString(sccLocationId.getBytes()) : StringUtils.EMPTY;
    ...
    private byte[] createJWTAuthenticationRequest() throws IOException {
        try (ByteArrayOutputStream byteArraysStream = new ByteArrayOutputStream()) {
            ...
            byteArraysStream.write(ByteBuffer.allocate(4).putInt(jwtToken.getBytes(UTF_8).length).array());
            byteArraysStream.write(jwtToken.getBytes(UTF_8));
            byteArraysStream.write(ByteBuffer.allocate(1).put((byte) sccLocationId.getBytes(UTF_8).length).array());
            byteArraysStream.write(sccLocationId.getBytes(UTF_8));
            return byteArraysStream.toByteArray();
        }
    }
    ...
    private byte[] createConnectCommandRequest(InetSocketAddress endpoint) throws IOException {
        ...
        try (ByteArrayOutputStream byteArraysStream = new ByteArrayOutputStream()) {
            ...
            if (hostToIPv4 != null) {
                ...
                byteArraysStream.write(ByteBuffer.allocate(1).put((byte) host.getBytes(UTF_8).length).array());
                byteArraysStream.write(host.getBytes(UTF_8));
            }
